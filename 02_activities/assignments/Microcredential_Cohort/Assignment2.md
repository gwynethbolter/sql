...
```
architecture 1: overwrite customer addresses (street, street_number, postal_code, etc) when the customer address no longer matches the customer_id. 

architecture 2: when the customer address doesn't match the customer_id, mark the old data as "obsolete" and create a new row with the customer_id and the new customer address (not maked as obsolete).
```
