...
WITH recent_visit as(
	...
	customer_id
	,market_date
	,dense_rank() OVER(PARTITION BY customer_id ORDER BY market_date DESC) as visit_recency
...
,market_date
,product_id
,count(product_id) OVER(PARTITION BY customer_id, product_id ORDER BY market_date) as times_bought_product
FROM customer_purchases
WHERE market_date < '2022-04-29';
...
SELECT 'Highest' AS label, market_date, total_sales FROM ranked_sales WHERE best_day_rank = 1
UNION
SELECT 'Lowest' AS label, market_date, total_sales FROM ranked_sales WHERE worst_day_rank = 1;
...
SELECT 
    v.vendor_name,
    p.product_name,
    SUM(up.original_price * 5) as potential_revenue
...
INSERT INTO product_units(
    ...
    product_qty_type, 
    snapshot_timestamp)
VALUES(100,'cheese popcorn','large', 2, 'unit', CURRENT_TIMESTAMP);
--SELECT * FROM product_units;
